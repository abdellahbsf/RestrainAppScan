...
type ClientFormGroupContent = {
  ...
  keyId: FormControl<IClient['keyId']>;
  keySecret: FormControl<IClient['keySecret']>;
  token: FormControl<IClient['token']>;
};
...
export class ClientFormService {
  createClientFormGroup(client: ClientFormGroupInput = { id: null }): ClientFormGroup {
    ...
    return new FormGroup<ClientFormGroupContent>({
      ...
      keyId: new FormControl(clientRawValue.keyId),
      keySecret: new FormControl(clientRawValue.keySecret),
      token: new FormControl(clientRawValue.token),
    });
  }
